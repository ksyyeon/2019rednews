...
public class MapActivity extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final LocationListener gpsLocationListener = new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                ...
                addMarker(lat, lng);  //위치가 갱신되면 사용자 위치 새 마커찍기
            }
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        addMarker(lat,lng); //지도 실행시 사용자 마커 찍기
    ...
    public void addMarker(double lat, double lng) {
        LatLng  myLocation = new LatLng(lat, lng); //실시간으로 전송된 선언된 위도, 경도 가져오기

        MarkerOptions myMarker = new MarkerOptions()
                .position(myLocation);
        //비트맵 사용
        BitmapDrawable myloc =(BitmapDrawable)getResources().getDrawable(R.drawable.placeholder);
        Bitmap c = myloc.getBitmap();
        Bitmap size = Bitmap.createScaledBitmap(c, 100, 100, true);
        myMarker.icon(BitmapDescriptorFactory.fromBitmap(size));
        mMap.addMarker(myMarker); //사용자 현재 위치
    }
}